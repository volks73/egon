...
//	insertValues = ['test', '11-1111-1-1-1', 'test description'],
	...
	updateExpr = SQL.expr().column('job_order_numbers_id').equals(1),
	selectColumns = ['alias', 'account_number', 'description'],
	selectExpr = SQL.expr().column('job_order_numbers_id').equals(1),
	insert = SQL.insert(tableName).columns(insertColumns).values(insertValues),
	update = SQL.update(tableName).set(updateValues).where(updateExpr),
	select = SQL.select(selectColumns).from(tableName).where(selectExpr);
